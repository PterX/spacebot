...
Agent tools and autonomous execution validate transitions. They can't skip steps or go backwards except to `backlog`, which is always allowed as a re-shelve action.
...
The UI, CLI, and operator API can set any status directly. This lets a human reconcile work completed outside Spacebot without fabricating approval or execution history. Directly marking a task done sets `completed_at`; it does not set `approved_at` unless the task was actually approved.
...
- **Activity timeline** — the detail view doesn't show a history of status changes, approvals, and worker events.
- **Task count badge** — the Tasks tab doesn't show a badge with the pending approval count yet.
- **Task archival** — done tasks accumulate indefinitely. Options: auto-archive after N days, a separate `archived` status, or UI filtering.
