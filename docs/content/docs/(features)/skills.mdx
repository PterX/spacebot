...
Skills are packaged instructions and workflows that give agents domain-specific knowledge. They're markdown files with typed frontmatter, procedural guidance, and optional bundled resources.
...
- **Categories and ranking** — channels receive a compact category index ranked by usage
- **Protected origins** — built-in and installed skills cannot be overwritten by agent reflection

### Reflection

After successful multi-step work, the next eligible persistence pass can turn the procedure into a new agent-authored skill or improve an existing agent-authored skill. Reflection is rate-limited per conversation and only writes within its origin boundary. Installed and built-in skills remain read-only to the agent.
...
skills/builtin/           // Built-in skill SKILL.md files (compiled into binary)
```
