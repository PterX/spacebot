...
Spacebot separates the concerns. The cortex runs per-agent maintenance and operational coordination. Cron jobs are user-defined recurring tasks, each with its own interval, prompt, and delivery target. When a cron job fires, it gets a fresh short-lived channel with full branching and worker capabilities. Multiple cron jobs run concurrently without blocking each other or the cortex.
