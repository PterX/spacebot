...
### 3. Ingestion and maintenance

File ingestion can save structured memories from imported material. Memory saves perform near-duplicate checks and can consolidate related entries before they enter an overgrown partition. Periodic maintenance handles decay, pruning, and association work.
...
When a channel starts, Spacebot renders a bounded deterministic view of the memory store into the system prompt. It groups current memory records by type and includes active goals. The render does not need an LLM synthesis pass, so it reflects the store on every turn.
...
- **Associations** -- create graph links for related memories

Near-duplicate consolidation also happens at write time, before maintenance is required. Maintenance runs in the cortex loop and does not block channels.
