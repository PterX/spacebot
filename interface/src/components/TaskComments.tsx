...
import {api, type TaskComment, type TaskAuthorKind} from "@/api/client";
import {useLiveContext} from "@/hooks/useLiveContext";

const PAGE_SIZE = 50;

/** Mirrors MAX_COMMENT_BODY_BYTES and MIN_COMMENT_BODY_CHARS on the server. */
const MAX_BODY_BYTES = 4000;
const MIN_BODY_CHARS = 4;
...
const AUTHOR_VARIANT: Record<TaskAuthorKind, "info" | "success" | "default"> = {
	user: "info",
	agent: "success",
	worker: "default",
	system: "default",
...
/**
 * A worker-authored comment links back to the run that produced it. The body
 * stays the worker's summary; the full output is fetched only when asked for.
 */
...
function CommentRow({
	...
	return (
		<li
			className={`border-b border-app-line/40 py-2.5 last:border-b-0 ${
				comment.author_type === "system" ? "opacity-70" : ""
			}`}
		>
			<div className="mb-1 flex items-center gap-2">
				<Badge variant={AUTHOR_VARIANT[comment.author_type]} size="sm">
					<FontAwesomeIcon
						icon={AUTHOR_ICON[comment.author_type]}
						className="text-[10px]"
					/>
					<span>{author}</span>
				</Badge>
...
export function TaskComments({
	...
	const {taskCommentVersion} = useLiveContext();
	const queryKey = ["task-comments", taskNumber];

	// A comment from an agent or worker arrives over SSE.
	const previousVersion = useRef(taskCommentVersion);
	useEffect(() => {
		if (taskCommentVersion !== previousVersion.current) {
			previousVersion.current = taskCommentVersion;
			void queryClient.invalidateQueries({queryKey});
		}
	}, [taskCommentVersion, queryClient, taskNumber]);
	...
	}, [canSubmit, trimmed, createMutation]);
	...
	return (
		<div className="border-t border-app-line/40 px-4 py-3">
			...
			{isLoading ? (
				...
				<p className="text-xs text-red-400">Failed to load the discussion.</p>
			) : comments.length === 0 ? (
			...
			<div className="mt-2">
				<textarea
					...
					aria-label={`Comment on task #${taskNumber}`}
					rows={3}
				...
				<div className="mt-1.5 flex items-center justify-between">
					...
					<Button size="sm" disabled={!canSubmit || createMutation.isPending} onClick={handleSubmit}>
