...
/// Whether a provider rejected the request because the tool calls and results
/// in the submitted history do not form a valid protocol sequence.
///
/// These 400s are deterministic for an unchanged request: the rejection lands
/// before the model runs, so a retry of the same history fails identically.
/// Recovery is only possible after the history itself changes.
...
mod tests {
    ...
    fn is_tool_history_mismatch_error_detects_provider_400s() {
        // The rejection that took down a live worker.
        assert!(is_tool_history_mismatch_error(
            "OpenAI ChatGPT Responses API error (400 Bad Request): No tool call found for \
             function call output with call_id call_HPJ4d0Mb42LJt6JzCqYcwRsq"
        ));
        // Anthropic phrasing for the mirror-image failure.
        assert!(is_tool_history_mismatch_error(
        ...
        assert!(is_tool_history_mismatch_error(
            "Invalid parameter: messages with role 'tool' must be a response to a preceding \
             message with 'tool_calls'. tool_call_id call_9 did not have a response message."
        ));

        // Other 400s must not route into history repair.
        assert!(!is_tool_history_mismatch_error("400 Bad Request"));
        assert!(!is_tool_history_mismatch_error(
            "context length exceeded: 210000 tokens"
        ));
    }
