...
spacebot restart   # in-place restart (falls back to stop + start)
...
An in-place restart re-execs the daemon with its original environment, so it doesn't pick up new environment variables or a rebuilt service definition. The stop + start fallback launches a fresh process and does pick those up — as does an explicit `spacebot stop && spacebot start` or `systemctl restart` under systemd. Agents can also restart the daemon themselves via the `restart` tool, and the dashboard has a Restart button under Settings → Updates.
...
<Cards>
  <Card title="Desktop app" href="/docs/desktop" />
