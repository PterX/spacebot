...
| `WorkerText` | Hook | Channel, UI | Streaming token-by-token text deltas |
| `SettingsUpdated` | API | Channel | Hot-reload channel settings |
...
| `cortex_events` | Cortex action log for maintenance and operational coordination |
| `cortex_chat_messages` | Persistent admin chat with cortex |
| `tasks` | Instance-wide task records, plans, dependencies, and execution state |
| `ingestion_progress` | Chunk-level progress for file ingestion |
| `agent_profile` | Cortex-generated personality data |
| `portal_conversations` | Portal chat conversation persistence with settings |
| `channel_settings` | Per-channel settings (response mode, model overrides, memory mode) |
| `token_usage` | Per-process token counts, model, provider, estimated cost |

A separate **global database** (`~/.spacebot/data/global.db`) holds instance-wide data shared across all agents:

| Table | Purpose |
|-------|---------|
| `projects` | Registered project directories with metadata |
| `project_repos` | Git repositories within projects |
| `project_worktrees` | Git worktrees linked to repos |
| `notifications` | Actionable events (task approvals, worker failures) |
| `wiki_pages` | Knowledge base pages with versioning |
| `wiki_page_versions` | Full edit history for wiki pages |
| `wiki_pages_fts` | FTS5 virtual table for wiki search |

Migrations are in `migrations/` (per-agent) and `migrations/global/` (instance-wide). Both are **immutable once committed**. Schema changes always go in new migration files. See [Memory](/docs/memory) for the memory graph schema.
...
| Cortex | 10 | Administrative cortex-chat turns. |
...
| Cortex | `/api/cortex` | Cortex chat and operational inspection |
...
| Portal | `/api/portal` | Chat interface, conversations, attachments |
| Models | `/api/models` | Available model listing |
| Topology | `/api/topology` | Full communication graph |
| Wiki | `/api/wiki` | Knowledge base pages and search |
| Notifications | `/api/notifications` | Actionable events and inbox |
| Projects | `/api/projects` | Project directories, repos, worktrees |
| Usage | `/api/usage` | Token usage and cost tracking |
| Secrets | `/api/secrets` | Credential store CRUD and encryption |
...
CLI (clap) → parse args
  ...
  → run()
    ...
    → Optional startup warmup pass (embedding readiness)
    → Initialize MessagingManager (start all platform adapters)
    → Initialize CronScheduler
    → Start Cortex loops (maintenance, associations, health observations)
...
├── api/                 — 30+ Axum endpoint modules
├── identity/            — identity file loading
├── secrets/             — secret store, OS keystore, output scrubbing
├── settings/            — key-value settings
├── tasks/               — task board
├── links/               — communication graph types
├── skills/              — skill management (installed + built-in)
├── wiki/                — wiki store, versioning, edit matching
├── notifications/       — notification store, emission
