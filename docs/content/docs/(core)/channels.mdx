...
| `bulletin` | TEXT | Reserved legacy column; current memory context renders from the store at turn time |
...
Each channel can be configured independently with per-channel settings: response mode, model overrides, memory mode, delegation mode, and worker context. Settings are stored in the `channel_settings` table and resolved at runtime with a priority chain: per-conversation > per-channel > binding defaults > system defaults.

See [Configuring Channels](/docs/configuring-channels) for the full settings reference and UI guide.

### Settings Resolution

```
ConversationSettings::resolve(conversation, channel, agent_default)
  → per-conversation overrides (portal settings panel, slash commands)
  → per-channel overrides (channel_settings table, cog icon in UI)
  → binding defaults (binding.settings in config)
  → system defaults (Active, Full memory, Standard delegation)
```

Implementation: `src/conversation/settings.rs` — `ResolvedConversationSettings::resolve()`, `src/conversation/channel_settings.rs` — per-channel persistence.

## Reserved Columns

Two columns exist in the schema but aren't populated yet:

**`bulletin`** — Reserved legacy column. Channels currently receive a deterministic memory-store render assembled at turn time, rather than a persisted per-channel bulletin.
