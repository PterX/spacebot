...
rollup_threshold = 12             # unrolled checkpoints before a level-1 rollup
rollup_batch = 8                  # oldest checkpoints summarized by one rollup
...
# Goal-oriented background work. `off` disables autonomous runs.
[defaults.autonomy]
level = "off"                 # "off", "observe", "suggest", or "act"
interval_secs = 1800
max_turns = 20
max_tasks_per_run = 2
timeout_secs = 600
...
# Durable autonomy wakes. Each wake sets exactly one trigger:
# `schedule`/`interval_secs`, `webhook = true`, or a typed `event`.
[[agents.wakes]]
id = "morning-review"
name = "Morning review"
schedule = "0 9 * * 1-5"
instructions = "Review active goals and ready tasks."
min_level = "observe"
...
webhook = true
instructions = "Inspect the deployment payload and create work only when needed."
min_level = "suggest"
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `mode` | string | `rolling` | `rolling` summaries or durable `chronicle` checkpoints |
...
| `worker_wall_clock_timeout_secs` | integer | 600 | Maximum duration of one worker run |
