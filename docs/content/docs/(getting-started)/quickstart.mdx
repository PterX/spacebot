...
Restarts re-exec the daemon with its original environment — they don't pick up new environment variables or a rebuilt service definition. Use a full stop + start (or `systemctl restart` under systemd) for that. Agents can also restart the daemon themselves via the `restart` tool, and the dashboard has a Restart button under Settings → Updates.
